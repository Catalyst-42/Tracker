...
При изменении времени предыдущего этапа можно пользоваться константами для часа, минуты и секунды, синтаксис общего выражения такой же как и для обычного калькулятора. Также изменение времени просиходит по следующему принципу: последний этап начался на `??` секунд позже. 
