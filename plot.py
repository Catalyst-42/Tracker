...
fig, axs = plt.subplot_mosaic(
    [["main"], ["average"]], 
    figsize = (PLOT_WIDTH, PLOT_START_HEIGTH+ALL_EXPERIMENT_TIME//(24*h)*PLOT_HEIGTH_STEP), 
    gridspec_kw = {"height_ratios": [ALL_EXPERIMENT_TIME//(24*h), 1]}
)
...
for i in range(len(save.activities_log)):
    ...
    if i != len(save.activities_log) - 1:
        next_activity_time = datetime.strptime(save.activities_log[i+1][1], "%H:%M:%S")
